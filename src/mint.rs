...
pub fn validate_period(e: &Env, period: u64) {
    ...
    if !(MIN_PERIOD_YEAR..=MAX_PERIOD_YEAR).contains(&year)
        || !(1..=12).contains(&month)
    {
...
/// Updates the latest period for a user if `period` is greater than the
/// currently stored latest period, or if no period has been recorded yet.
///
/// Storage bytes are accounted once, when the entry is first created;
/// subsequent updates only overwrite the existing value and renew its TTL.
...
pub(crate) fn mint_wrap_batch(
    ...
    if let Some(agg_sig) = aggregated_signature {
        ...
        if let Err(err) =
            crate::signature::verify_batch_aggregated_signature(
                &e,
                &admin_pubkey,
                &contract_id,
                &items,
    ...
    for item in items.iter() {
        ...
        let user_periods_key =
            DataKey::UserPeriods(item.user.clone());
        ...
        let wrap_periods_key =
            DataKey::WrapPeriods(item.user.clone());
...
pub(crate) fn expire_wrap(
    ...
    if !record
        .fsm
        .transition_to(WrapState::Expired, now)
    {
        panic_with_error!(
            e,
            ContractError::InvalidStateTransition
